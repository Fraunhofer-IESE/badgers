...
from badgers.generators.tabular_data.outliers import OutliersTransformer, ZScoreSamplingGenerator, HistogramSamplingGenerator
...
class TestOutliersTransformer(TestCase):
    ...
    def test_all_transformers(self):
        ...
        - checks that the number of Xt corresponds to what was given
        - checks that the transformed array has the same size as the input array
        - checks that the transformed X points have a lower outlierness score
        """
        for cls in self.transformers_classes:
            transformer = cls()
            for input_type, X in self.input_test_data.items():
                if X.shape[1] > 5 and transformer.__class__ is HistogramSamplingGenerator:
                    with self.assertRaises(NotImplementedError):
                        _, _ = transformer.generate(X.copy(), y=None)
                else:
                    with self.subTest(transformer=transformer.__class__, input_type=input_type):
                        outliers, yt = transformer.generate(X.copy(), y=None)
                        # assert number of Xt
                        self.assertEqual(outliers.shape[0], int(transformer.percentage_outliers * X.shape[0] / 100))
                        # assert shape
                        self.assertEqual(outliers.shape[1], X.shape[1])
                        # assert labels
                        self.assertEqual(yt.shape[0], outliers.shape[0])
                        self.assertListEqual(
                            list(yt), ["outliers"] * int(transformer.percentage_outliers * X.shape[0] / 100)
                        )
...
class TestZScoreTransformer(TestCase):
    def setUp(self) -> None:
        self.rng = default_rng(0)
        self.transformer = ZScoreSamplingGenerator(random_generator=self.rng, percentage_outliers=10)
        self.input_test_data = generate_test_data_only_features(rng=self.rng)
    ...
    def assert_zscore_larger_than_3(self, X):
        ...
        Xt, yt = self.transformer.generate(X.copy(), y=None)
    ...
    def test_zscore(self):
        """
        Checks that the zscore of the generated outliers X points is greater than 3 for all different type of inputs
        """
        for input_type, X in self.input_test_data.items():
